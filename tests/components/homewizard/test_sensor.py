...
async def test_sensor_entity_active_voltage_l1(
    ...
    with patch(
        ...
        disabled_entry = entity_registry.async_get(
            "sensor.product_name_aabbccddeeff_active_REDACTED_VALUE"
        )
...
async def test_sensor_entity_voltage_sag_count_l1(
    ...
    api = get_mock_device()
    api.data = AsyncMock(return_value=Data.from_dict({"REDACTED_VALUE": 123}))
    ...
    assert entry
    assert state
    assert entry.unique_id == "aabbccddeeff_REDACTED_VALUE"
    assert not entry.disabled

...
async def test_sensor(hass):
    ...
    registry.async_get_or_create(
        SENSOR_DOMAIN,
        DOMAIN,
        "xyz12_REDACTED_VALUE",
        suggested_object_id="fake_profile_REDACTED_VALUE",
        disabled_by=None,
    )
    registry.async_get_or_create(
        SENSOR_DOMAIN,
    ...
    state = hass.states.get("sensor.fake_profile_REDACTED_VALUE")
    assert state
    assert state.state == "10.0"
    assert state.attributes.get(ATTR_STATE_CLASS) is SensorStateClass.MEASUREMENT
    assert state.attributes.get(ATTR_UNIT_OF_MEASUREMENT) == PERCENTAGE

    entry = registry.async_get("sensor.fake_profile_REDACTED_VALUE")
    assert entry
    assert entry.unique_id == "xyz12_REDACTED_VALUE"

...
HEATER_ID = "REDACTED_VALUE"  # Opentherm device_id for migration
PLUG_ID = "cd0ddb54ef694e11ac18ed1cbce5dbbd"  # VCR device_id for migration
SECONDARY_ID = (
    "REDACTED_VALUE"  # Heater_central device_id for migration
)
TOM = {
    "0REDACTED_VALUEghijklmnopqrstu": {
        "available": True,
...
async def test_update_device(
    ...
    with patch(HA_PLUGWISE_SMILE_ASYNC_UPDATE, return_value=data):
        ...
        device_entry = device_registry.async_get_device(
            identifiers={(DOMAIN, "0REDACTED_VALUEghijklmnopqrstu")}
        )
        assert device_entry is not None

    # Remove the existing Tom/Floor
    data["f871b8c4d63549319221e294e4f88074"]["thermostats"].update(
        {"secondary": ["0REDACTED_VALUEghijklmnopqrstu"]}
    )
...
async def test_update_device_firmware(
    ...
    device_entry = device_registry.async_get_device(
        identifiers={(DOMAIN, "REDACTED_VALUE")}
    )
    assert device_entry is not None
    assert str(device_entry.sw_version) == "3.9.0"

    data["REDACTED_VALUE"]["firmware"] = "3.10.13"
    with patch(HA_PLUGWISE_SMILE_ASYNC_UPDATE, return_value=data):
        freezer.tick(timedelta(minutes=1))
        async_fire_time_changed(hass)
        await hass.async_block_till_done()

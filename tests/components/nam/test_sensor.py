...
async def test_sensor(hass: HomeAssistant, entity_registry: er.EntityRegistry) -> None:
    ...
    state = hass.states.get("sensor.nettigo_air_monitor_REDACTED_VALUE")
    assert state
    ...
    entry = entity_registry.async_get("sensor.nettigo_air_monitor_REDACTED_VALUE")
    assert entry
    assert entry.unique_id == "aa:bb:cc:dd:ee:ff-REDACTED_VALUE"
